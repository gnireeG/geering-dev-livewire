<div class="flex justify-center">
    <div class="container" x-data="{showTags: $persist(false)}">
        <div class="mb-8">
            <h1>Portfolio</h1>
            <p class="text-muted">{{ __('portfolio.intro')}}</p>
            <div class="mt-8">
                <button x-show="!showTags" @click="showTags = !showTags" class="flex gap-2 cursor-pointer items-center hover:text-primary">{{__('portfolio.show_tags')}}<flux:icon.chevron-down variant="mini" /></button>
                <button x-show="showTags" @click="showTags = !showTags" class="flex gap-2 cursor-pointer items-center hover:text-primary">{{__('portfolio.hide_tags')}}<flux:icon.chevron-up variant="mini" /></button>
            ...
            @foreach($this->portfolios as $portfolio)
                ...
                class="cursor-pointer bg-bg-light rounded-t-2xl rounded-b-2xl group">
                    ...
                    <div class="p-4 overflow-hidden">
                        <h4 class="text-primary/80 group-hover:text-primary transition-colors font-bold text-lg">{{ $portfolio->title }}</h3>
                        <p class="text-base">{{ $portfolio->shortdesc }}</p>
                        <div class="text-sm flex gap-1 mt-4 flex-wrap" x-show="showTags" x-transition >
